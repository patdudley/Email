...
const taskIntegrations:Record<TaskIntegration,{name:string;mark:string;embedUrl:string;description:string}>={
  online_presence:{name:"SEO optimization",mark:"SEO",embedUrl:"https://locallift-audit.patduds.chatgpt.site",description:"Run a verified Google listing and website audit inside Resolve."},
  drybar_payroll:{name:"Payroll converter",mark:"PC",embedUrl:"https://drybar-payroll-converter.patduds.chatgpt.site",description:"Convert Booker reports into a human-reviewed Paychex import inside Resolve."},
};
...
export default function Home(){
  ...
  const [taskSettingsTitle,setTaskSettingsTitle]=useState("");
  const [taskSettingsDescription,setTaskSettingsDescription]=useState("");
  const [taskSettingsDeadline,setTaskSettingsDeadline]=useState("");
  const [taskSettingsRecurrenceType,setTaskSettingsRecurrenceType]=useState<"one_time"|"recurring">("one_time");
  const [taskSettingsRecurrenceEvery,setTaskSettingsRecurrenceEvery]=useState(1);
  const [taskSettingsRecurrenceUnit,setTaskSettingsRecurrenceUnit]=useState<"day"|"week"|"month">("week");
  const [taskSettingsScheduleTime,setTaskSettingsScheduleTime]=useState("09:00");
  ...
  async function openTask(id:string){
    setActiveTaskId(id);setCreatingTask(false);setTaskSettingsOpen(false);setTaskMessages([]);setTasksLoading(true);
    try{
  ...
  function applyTaskTemplate(integration:TaskIntegration|null){
    ...
    if(integration==="online_presence"){
      setNewTaskTitle("SEO optimization");setNewTaskDescription("Audit the business’s Google listing and website, identify verified issues, and organize the highest-impact SEO actions. Business name and website: ");setNewTaskRecurrenceType("recurring");setNewTaskRecurrenceEvery(1);setNewTaskRecurrenceUnit("month");setNewTaskScheduleTime("09:00");
    }else if(integration==="drybar_payroll"){
      setNewTaskTitle("Payroll converter");setNewTaskDescription("Process the manager-corrected Booker payroll report and matching timeclock report, resolve every blocking exception, review totals, and prepare the Paychex SPI CSV for human approval. Shop and pay period: ");setNewTaskRecurrenceType("recurring");setNewTaskRecurrenceEvery(2);setNewTaskRecurrenceUnit("week");setNewTaskScheduleTime("09:00");
    }else{
      setNewTaskTitle("");setNewTaskDescription("");setNewTaskRecurrenceType("one_time");setNewTaskRecurrenceEvery(1);setNewTaskRecurrenceUnit("week");setNewTaskScheduleTime("09:00");
    }
  }
  async function createTask(input?:{title:string;description:string;deadline:string|null;recurrenceType:"one_time"|"recurring";recurrenceEvery:number|null;recurrenceUnit:"day"|"week"|"month"|null;scheduleTime?:string|null;sourceThreadId?:string;integrationType?:TaskIntegration|null}){
    ...
    try{
      const response=await fetch("/api/tasks",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({title,description,deadline:input?.deadline??(newTaskDeadline||null),recurrenceType:input?.recurrenceType??newTaskRecurrenceType,recurrenceEvery:input?.recurrenceEvery??newTaskRecurrenceEvery,recurrenceUnit:input?.recurrenceUnit??newTaskRecurrenceUnit,scheduleTime:input?.scheduleTime??(newTaskRecurrenceType==="recurring"?newTaskScheduleTime:null),sourceThreadId:input?.sourceThreadId,integrationType:input?(input.integrationType??null):newTaskIntegration})});
      ...
      setSavedTasks(current=>[json.task!,...current]);setActiveTaskId(json.task.id);setTaskFilter("active");setCreatingTask(false);setView("tasks");setOpenId(null);setNewTaskTitle("");setNewTaskDescription("");setNewTaskDeadline("");setNewTaskRecurrenceType("one_time");setNewTaskRecurrenceEvery(1);setNewTaskRecurrenceUnit("week");setNewTaskScheduleTime("09:00");setNewTaskIntegration(null);setTaskMessages([]);
  ...
  function openTaskSettings(task:TaskRecord){
    setTaskSettingsTitle(task.title);setTaskSettingsDescription(task.description);setTaskSettingsDeadline(task.deadline??"");setTaskSettingsRecurrenceType(task.recurrenceType);setTaskSettingsRecurrenceEvery(task.recurrenceEvery??1);setTaskSettingsRecurrenceUnit(task.recurrenceUnit??"week");setTaskSettingsScheduleTime(task.scheduleTime??"09:00");setTaskSettingsOpen(true);
  }
  async function saveTaskSettings(task:TaskRecord){
    const title=taskSettingsTitle.trim();const description=taskSettingsDescription.trim();
    if(!title||!description){notify("Add a title and instructions");return}
    setTaskSettingsSaving(true);
    try{
      const response=await fetch("/api/tasks",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:task.id,title,description,deadline:taskSettingsDeadline||null,recurrenceType:taskSettingsRecurrenceType,recurrenceEvery:taskSettingsRecurrenceEvery,recurrenceUnit:taskSettingsRecurrenceUnit,scheduleTime:taskSettingsRecurrenceType==="recurring"?taskSettingsScheduleTime:null})});
      const json=await response.json() as {task?:TaskRecord;error?:string};
      if(!response.ok||!json.task)throw new Error(json.error??"Could not save task settings");
      setSavedTasks(current=>current.map(item=>item.id===task.id?{...item,...json.task}:item));setTaskSettingsOpen(false);notify("Task settings saved");
    }catch(error){notify(error instanceof Error?error.message:"Could not save task settings")}finally{setTaskSettingsSaving(false)}
  }
  ...
  return <main className={`app ${view==="tasks"?"tasks-mode":""}`}>
    ...
    <section className="main">
      ...
      {view==="tasks"&&<section className="tasks task-workspace">
        ...
        {creatingTask?<div className="new-task-page"><form onSubmit={event=>{event.preventDefault();void createTask()}}>
          <span className="eyebrow">NEW TASK</span><h1>What are you working on?</h1><p>Give Resolve the goal and enough background to start. It will ask focused follow-up questions before researching and building your plan.</p>
          <fieldset className="task-templates"><legend>Quick starts <span>Optional</span></legend><div><button type="button" title="Start a custom task" className={newTaskIntegration===null?"active":""} onClick={()=>applyTaskTemplate(null)}><i>＋</i><span><b>Custom task</b><small>Start from scratch</small></span></button>{(Object.entries(taskIntegrations) as Array<[TaskIntegration,(typeof taskIntegrations)[TaskIntegration]]>).map(([id,integration])=><button type="button" title={integration.description} key={id} className={newTaskIntegration===id?"active":""} onClick={()=>applyTaskTemplate(id)}><i>{integration.mark}</i><span><b>{integration.name}</b><small>{integration.description}</small></span></button>)}</div>{newTaskIntegration&&<p className="task-template-help">{taskIntegrations[newTaskIntegration].description} You can edit every field below.</p>}</fieldset>
          <label>Title<input autoFocus value={newTaskTitle} onChange={event=>setNewTaskTitle(event.target.value)} placeholder="e.g. Plan a customer advisory board" maxLength={200}/></label>
          <label>Instructions<textarea value={newTaskDescription} onChange={event=>setNewTaskDescription(event.target.value)} placeholder="What outcome do you want? What should Resolve know?" maxLength={5000}/></label>
          <fieldset className="task-schedule"><legend>Schedule</legend><div className="schedule-toggle"><button type="button" className={newTaskRecurrenceType==="one_time"?"active":""} onClick={()=>setNewTaskRecurrenceType("one_time")}>One-time</button><button type="button" className={newTaskRecurrenceType==="recurring"?"active":""} onClick={()=>setNewTaskRecurrenceType("recurring")}>↻ Recurring</button></div>{newTaskRecurrenceType==="recurring"&&<div className="recurrence-fields"><span>Repeat every</span><input aria-label="Recurrence interval" type="number" min="1" max="365" value={newTaskRecurrenceEvery} onChange={event=>setNewTaskRecurrenceEvery(Math.min(365,Math.max(1,Number(event.target.value)||1)))}/><select aria-label="Recurrence unit" value={newTaskRecurrenceUnit} onChange={event=>setNewTaskRecurrenceUnit(event.target.value as "day"|"week"|"month")}><option value="day">day(s)</option><option value="week">week(s)</option><option value="month">month(s)</option></select><label>at <input aria-label="Run time" type="time" value={newTaskScheduleTime} onChange={event=>setNewTaskScheduleTime(event.target.value)}/></label></div>}</fieldset>
          <label className="deadline-field">Deadline <span>Optional</span><input type="date" value={newTaskDeadline} onChange={event=>setNewTaskDeadline(event.target.value)}/></label>
          <button className="create-task-primary" disabled={taskAgentLoading||!newTaskTitle.trim()||!newTaskDescription.trim()}>{taskAgentLoading?"Starting…":"Create task and start →"}</button>
        </form></div>
        :activeTask?<div className={`task-chat-page ${activeTask.integrationType?"has-native-workflow":""}`}>
          <header><div><span className="eyebrow">TASK WORKSPACE</span><h1>{(activeTask.status==="completed"||activeTask.currentPeriodCompletedAt)&&<span className="completed-title-check">✓</span>}{activeTask.title}</h1><p>{activeTask.description}</p><div className="task-meta"><span>{activeTask.recurrenceType==="recurring"?`↻ Every ${activeTask.recurrenceEvery} ${activeTask.recurrenceUnit}${activeTask.recurrenceEvery===1?"":"s"}${activeTask.scheduleTime?` at ${activeTask.scheduleTime}`:""}`:"One-time"}</span><span>{activeTask.deadline?`Due ${new Date(`${activeTask.deadline}T00:00:00`).toLocaleDateString(undefined,{month:"long",day:"numeric",year:"numeric"})}`:"No deadline"}</span>{activeTask.currentPeriodCompletedAt&&<span className="task-period-complete">✓ {taskCompletionLabel(activeTask)}</span>}{activeTask.sourceThreadId&&<span>Created from email</span>}</div>{activeTask.integrationType&&<div className="task-integration"><span>{taskIntegrations[activeTask.integrationType].mark}</span><div><b>{taskIntegrations[activeTask.integrationType].name}</b><small>Runs here inside Resolve</small></div></div>}{activeTask.completionEvidence&&<div className="task-evidence"><b>✓ Completed automatically from email</b><span>“{activeTask.completionEvidence.subject??"Payment confirmation"}”{activeTask.completionEvidence.sender?` from ${activeTask.completionEvidence.sender}`:""}</span>{activeTask.completionEvidence.summary&&<small>{activeTask.completionEvidence.summary}</small>}</div>}</div><div className="task-header-actions"><button className="task-settings-button" onClick={()=>openTaskSettings(activeTask)}>⚙ Settings</button><button className={(activeTask.status==="completed"||activeTask.currentPeriodCompletedAt)?"reopen-task":"complete-task"} onClick={()=>void updateTaskStatus(activeTask,(activeTask.status==="completed"||Boolean(activeTask.currentPeriodCompletedAt))?"active":"completed")}>{(activeTask.status==="completed"||activeTask.currentPeriodCompletedAt)?(activeTask.recurrenceType==="recurring"?"↻ Reopen this period":"↻ Reopen"):(activeTask.recurrenceType==="recurring"?"✓ Mark this period complete":"✓ Mark complete")}</button><button className="delete-task" onClick={()=>void deleteTask(activeTask.id)}>Delete</button></div></header>
          {taskSettingsOpen&&<form className="task-settings-panel" onSubmit={event=>{event.preventDefault();void saveTaskSettings(activeTask)}}><header><div><span className="eyebrow">TASK SETTINGS</span><h2>Edit task behavior</h2></div><button type="button" aria-label="Close task settings" onClick={()=>setTaskSettingsOpen(false)}>×</button></header><label>Title<input value={taskSettingsTitle} onChange={event=>setTaskSettingsTitle(event.target.value)} maxLength={200}/></label><label>Instructions<textarea value={taskSettingsDescription} onChange={event=>setTaskSettingsDescription(event.target.value)} rows={5} maxLength={5000}/></label><fieldset><legend>Schedule</legend><div className="schedule-toggle"><button type="button" className={taskSettingsRecurrenceType==="one_time"?"active":""} onClick={()=>setTaskSettingsRecurrenceType("one_time")}>One-time</button><button type="button" className={taskSettingsRecurrenceType==="recurring"?"active":""} onClick={()=>setTaskSettingsRecurrenceType("recurring")}>↻ Recurring</button></div>{taskSettingsRecurrenceType==="recurring"&&<div className="recurrence-fields"><span>Repeat every</span><input aria-label="Settings recurrence interval" type="number" min="1" max="365" value={taskSettingsRecurrenceEvery} onChange={event=>setTaskSettingsRecurrenceEvery(Math.min(365,Math.max(1,Number(event.target.value)||1)))}/><select aria-label="Settings recurrence unit" value={taskSettingsRecurrenceUnit} onChange={event=>setTaskSettingsRecurrenceUnit(event.target.value as "day"|"week"|"month")}><option value="day">day(s)</option><option value="week">week(s)</option><option value="month">month(s)</option></select><label>at <input aria-label="Settings run time" type="time" value={taskSettingsScheduleTime} onChange={event=>setTaskSettingsScheduleTime(event.target.value)}/></label></div>}</fieldset><label>Deadline <span>Optional</span><input type="date" value={taskSettingsDeadline} onChange={event=>setTaskSettingsDeadline(event.target.value)}/></label><footer><button type="button" onClick={()=>setTaskSettingsOpen(false)}>Cancel</button><button className="save-task-settings" disabled={taskSettingsSaving}>{taskSettingsSaving?"Saving…":"Save settings"}</button></footer></form>}
          {activeTask.integrationType&&<section className="native-workflow"><header><div><span>{taskIntegrations[activeTask.integrationType].mark}</span><div><b>{taskIntegrations[activeTask.integrationType].name}</b><small>Complete this workflow without leaving Resolve</small></div></div><em>Private task workspace</em></header><iframe title={`${taskIntegrations[activeTask.integrationType].name} workflow`} src={taskIntegrations[activeTask.integrationType].embedUrl} sandbox="allow-same-origin allow-scripts allow-forms allow-downloads allow-popups allow-popups-to-escape-sandbox"/></section>}
          <div className="task-conversation">{!taskMessages.length&&!taskAgentLoading&&<div className="task-agent-intro"><span>✦</span><p>Resolve is ready to review this task.</p><button onClick={()=>void runTaskAgent(activeTask.id,"",true)}>Start the conversation</button></div>}{taskMessages.map((message,index)=><article key={message.id??`${message.role}-${index}`} className={`task-message ${message.role}`}><div className="message-author">{message.role==="assistant"?<><span>✦</span>Resolve</>:"You"}</div><div className="message-content">{message.content.split("\n").map((line,lineIndex)=><span key={lineIndex}>{line||<br/>}</span>)}</div>{message.sources&&message.sources.length>0&&<div className="task-sources"><b>Sources</b>{message.sources.map(source=><a key={source.url} href={source.url} target="_blank" rel="noreferrer">{source.title} ↗</a>)}</div>}</article>)}{taskAgentLoading&&<div className="task-agent-thinking"><span/><span/><span/> Resolve is thinking and researching…</div>}</div><form className="task-chat-form" onSubmit={event=>{event.preventDefault();void runTaskAgent(activeTask.id,taskChatInput)}}><textarea value={taskChatInput} onChange={event=>setTaskChatInput(event.target.value)} placeholder="Answer Resolve or ask it to research, strategize, or organize the next step…" rows={2} onKeyDown={event=>{if(event.key==="Enter"&&!event.shiftKey){event.preventDefault();void runTaskAgent(activeTask.id,taskChatInput)}}}/><button aria-label="Send to Resolve" disabled={taskAgentLoading||!taskChatInput.trim()}>↑</button></form>
        </div>
