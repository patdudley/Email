...
export default function Home(){
  ...
  function scheduleRecipientSearch(value:string){
    ...
    recipientSearchTimer.current=window.setTimeout(()=>void loadRecipientSuggestions(query.length>=2?query:""),250);
  }
  ...
  async function createTask(input?:{title:string;description:string;deadline:string|null;recurrenceType:"one_time"|"recurring";recurrenceEvery:number|null;recurrenceUnit:"day"|"week"|"month"|null;sourceThreadId?:string}){
  ...
  async function updateTaskStatus(task:TaskRecord,status:"active"|"completed"){
    ...
    const json=await response.json() as {error?:string;updatedAt?:number};
    if(!response.ok){notify(json.error??"Could not update task");return}
    setSavedTasks(current=>current.map(item=>item.id===task.id?{...item,status,updatedAt:json.updatedAt??item.updatedAt}:item));
    setTaskFilter(status);notify(status==="completed"?"Task completed":"Task reopened");
  }
  ...
  async function askEmail(e:React.FormEvent){
    ...
    try{
      ...
      const json=await response.json() as {answer?:string;matches?:Mail[];nextPageToken?:string|null;error?:string;signInUrl?:string;upgradeRequired?:boolean;usage?:number;limit?:number};
      if(response.status===401){if(json.signInUrl)setSignInUrl(json.signInUrl);setAccountOpen(true);return}
      if(json.upgradeRequired){setAccountOpen(true);await loadAccount(false);notify(json.error??"Monthly AI limit reached");return}
      if(!response.ok||!json.answer){notify(json.error??"AI search is unavailable");return}
      const matches=json.matches??[];setView("mail");setOpenId(null);setSelected([]);setSearchMode(true);setGmailSearchQuery(question);setLiveMail(matches);setUnread(matches.filter(message=>message.unread).map(message=>message.id));setStarred(matches.filter(message=>message.starred).map(message=>message.id));setGmailPage(0);setGmailPageTokens(json.nextPageToken?["",json.nextPageToken]:[""]);setGmailNextPageToken(json.nextPageToken??null);setAnswer({text:json.answer,ids:matches.map(m=>m.id),query:question,count:matches.length});
      if(account&&typeof json.usage==="number")setAccount({...account,usage:json.usage,limit:json.limit??account.limit});
    ...
    <section className="main">
      ...
      {view==="tasks"&&<section className="tasks task-workspace">
        <div className="task-list"><header><h1>Tasks</h1><button aria-label="Create new task" onClick={()=>{setCreatingTask(true);setActiveTaskId(null);setTaskMessages([])}}>＋</button></header>
          ...
          {tasksLoading&&!savedTasks.length?<div className="task-list-loading">Loading tasks…</div>:visibleTasks.map(task=><article key={task.id} className={`${activeTaskId===task.id?"active ":""}${task.status==="completed"?"completed":""}`} onClick={()=>void openTask(task.id)}><span className={`task-status-dot ${task.status==="completed"?"complete":""}`}>{task.status==="completed"?"✓":""}</span><div><h2>{task.title}</h2><p>{task.description}</p><footer><span>{task.recurrenceType==="recurring"?`Every ${task.recurrenceEvery} ${task.recurrenceUnit}${task.recurrenceEvery===1?"":"s"}`:"One-time"}</span>{task.deadline&&<time>{new Date(`${task.deadline}T00:00:00`).toLocaleDateString(undefined,{month:"short",day:"numeric"})}</time>}</footer></div></article>)}
