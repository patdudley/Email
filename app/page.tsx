...
export default function Home(){
  ...
  return <main className="app">
    ...
    {compose&&(recipientContactsAuthorized===false||recipientContactsUnavailable)&&<aside className="contacts-permission-nudge"><div><b>{recipientContactsUnavailable?"Turn on Google People API":"Get Gmail-quality contact suggestions"}</b><span>{recipientContactsUnavailable?"Google Contacts permission is approved, but the People API is not available yet.":"Reconnect once so Resolve can search Google Contacts and Other contacts—not just email senders."}</span></div><button onClick={()=>window.location.assign("/api/connectors/google/start")}>{recipientContactsUnavailable?"Retry connection":"Enable contacts"}</button></aside>}
