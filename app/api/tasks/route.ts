...
export async function PATCH(request:Request) {
  ...
  const task=await db.prepare(`SELECT id,title,description,deadline,recurrence_type AS recurrenceType,recurrence_every AS recurrenceEvery,recurrence_unit AS recurrenceUnit,status,source_thread_id AS sourceThreadId,created_at AS createdAt,updated_at AS updatedAt FROM tasks WHERE id=? AND user_email=?`).bind(id,email).first<TaskRow>();
  if(!task)return Response.json({error:"Task not found"},{status:404});
