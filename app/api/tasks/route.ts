...
export async function GET(request:Request) {
  ...
  const db=getD1();
  const tasks=id
    ? await db.prepare(`SELECT id,title,description,deadline,status,source_thread_id AS sourceThreadId,created_at AS createdAt,updated_at AS updatedAt FROM tasks WHERE user_email=? AND id=?`).bind(user.email.toLowerCase(),id).all<TaskRow>()
    : await db.prepare(`SELECT id,title,description,deadline,status,source_thread_id AS sourceThreadId,created_at AS createdAt,updated_at AS updatedAt FROM tasks WHERE user_email=? ORDER BY updated_at DESC LIMIT 200`).bind(user.email.toLowerCase()).all<TaskRow>();
...
export async function POST(request:Request) {
  ...
  const body=await request.json() as {title?:string;description?:string;deadline?:string;sourceThreadId?:string};
  ...
  await ensureUser(user);
  const task:TaskRow={id:crypto.randomUUID(),title,description,deadline:cleanDeadline(body.deadline),status:"active",sourceThreadId:body.sourceThreadId?.trim()||null,createdAt:Math.floor(Date.now()/1000),updatedAt:Math.floor(Date.now()/1000)};
  await getD1().prepare(`INSERT INTO tasks (id,user_email,title,description,deadline,status,source_thread_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?)`).bind(task.id,user.email.toLowerCase(),task.title,task.description,task.deadline,task.status,task.sourceThreadId,task.createdAt,task.updatedAt).run();
